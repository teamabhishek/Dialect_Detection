...
def feat_extract(filename,feat_type,n_fft_length=512,hop=160,vad=True,cmvn=False,exclude_short=500):
    filelist = np.loadtxt(filename,delimiter='\t',dtype=str,usecols=(0))
    ...
    for index,wavname in enumerate(filelist):
        #read audio input
        y, sr = librosa.core.load(wavname,sr=16000,mono=True,dtype='float32')

        #extract feature
        if feat_type =='melspec':
            Y = librosa.feature.melspectrogram(y=y,sr=sr,n_fft=n_fft_length,hop_length=hop,n_mels=40,fmin=133,fmax=6955)
        elif feat_type =='mfcc':
            Y = librosa.feature.mfcc(y=y,sr=sr,n_fft=n_fft_length,hop_length=hop,n_mfcc=40,fmin=133,fmax=6955)
        elif feat_type =='spec':
            Y = np.abs( librosa.core.stft(y=y,n_fft=n_fft_length,hop_length=hop,win_length=400) )
        elif feat_type =='logspec':
            Y = np.log( np.abs( librosa.core.stft(y=y,n_fft=n_fft_length,hop_length=hop,win_length=400) ) )
        elif feat_type =='logmel':
            Y = np.log( librosa.feature.melspectrogram(y=y,sr=sr,n_fft=n_fft_length,hop_length=hop,n_mels=40,fmin=133,fmax=6955) )

        Y = Y.transpose()

        # Simple VAD based on the energy
        if vad:
            E = librosa.feature.rms(y=y, frame_length=n_fft_length,hop_length=hop)
...
for i in range(TOTAL_JOB):
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+ tffilename +'_train.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+tffilename+'_train_shuffle.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    
    
    
FILENAME = 'data_add/dev.txt'
feat, utt_label, utt_shape, tffilename = feat_extract(FILENAME,FEAT_TYPE,N_FFT,HOP,VAD,CMVN,EXCLUDE_SHORT)
print (tffilename)

TOTAL_JOB = 1
for i in range(TOTAL_JOB):
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+ tffilename +'_dev.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+tffilename+'_dev_shuffle.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)



####file load & extracting mel-spectrogram
FILENAME = 'data_add/test.txt'
EXCLUDE_SHORT = 0
...
TOTAL_JOB = 1
for i in range(TOTAL_JOB):
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+ tffilename +'_test.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = 'data_add/tfrecords/'+'mgb3_'+ tffilename +'_test_shuffle.'+str(i)+'.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
